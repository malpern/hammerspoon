...
local function generateSymbolHtml(symbol, color, size, font)
    -- Ensure symbol is properly encoded for HTML
    local encodedSymbol = symbol:gsub(".", function(c)
    ...
    return string.format([[
        <div style='
            ...
            justify-content: center;
        '>%s</div>
    ]], font.FAMILY, color, size, font.WEIGHT, encodedSymbol)
end
...
function M.generateArrowHtml(direction, keyType)
    ...
    local symbolHtml = string.format([[
        <div style='
            font-family: %s;
            color: %s;
            text-shadow: none;
            ...
            justify-content: center;
        '>%s</div>
    ]], font.FAMILY, symbolColor, font.SIZES.SYMBOL, font.WEIGHT, symbol)
    
    local letterHtml = string.format([[
        <div style='
            font-family: %s;
            color: %s;
            font-size: %s;
            ...
            justify-content: center;
        '>%s</div>
    ]], font.FAMILY, letterColor, font.SIZES.LETTER, font.WEIGHT, letter)
    
    -- Combine components in the correct order based on key type
    return string.format([[
        <div style='display: flex; flex-direction: column; align-items: center;'>
            %s%s
        </div>
