...
local hyperWatcher = hs.eventtap.new({hs.eventtap.event.types.keyDown}, function(event)
    ...
    if arrowKeyCode then
        ...
        -- Don't start a celebration window for Hyper+HJKL
        -- Instead, check if this matches a recent arrow press
        checkCelebration(direction, false)
        ...
        return true  -- Prevent the key from being passed to the system
    end
