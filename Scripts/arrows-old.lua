...
-- NOTE: THIS IS THE OLD ARROWS SCRIPT. I'M KEEPING IT HERE FOR REFERENCE.
-- TODO:
...
local function getArrowHtml(direction, isArrowKey)
    ...
    -- Special case for "back" command with smaller text
    if direction == "back" then
        return string.format(backTemplate, 
            "white",      -- Top symbol color (white triangle)
            [[<span style="font-size: 0.85em">▲</span>]],  -- 25% smaller triangle
            "#666666",   -- Bottom text color (gray)
            string.format([[<span style="font-size: 0.65em">Back</span>]])  -- Smaller "Back" text
        )
    end
...
-- Add near the top with other sound loading (around line 178)
local backSound = hs.sound.getByFile(configPath .. "up_deeper.wav")
if backSound then
    print("Successfully loaded sound for back")
else
    print("Error: Failed to load sound for back from path: " .. configPath .. "up_deeper.wav")
end
